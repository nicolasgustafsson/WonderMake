...
public:
    ...
    BaseObject& operator=(BaseObject&& aOther);
    BaseObject() = default;
    ~BaseObject() noexcept;
...
template<typename TType, typename TCreateFunc>
inline TType& BaseObject::Add(TCreateFunc aCreateFunc, DependencyDestructor& aDestructor, const bool aExplicitlyAdded)
{
...
template<typename TType>
inline void BaseObject::Remove(const bool aExplicitlyRemoved)
{
    if constexpr (std::is_base_of<SComponent, TType>::value)
...
template<typename TVisitFunc>
inline void BaseObject::Visit(TVisitFunc aVisitFunc)
{
...
template<typename TType, typename TBaseType, typename TCreateFunc>
inline TType& BaseObject::Add(PairList<TBaseType>& aList, TCreateFunc aCreateFunc, DependencyDestructor& aDestructor, const bool aExplicitlyAdded)
{
...
template<typename TType, typename TBaseType>
inline void BaseObject::Remove(PairList<TBaseType>& aList, const bool aExplicitlyRemoved)
{
    const auto it = std::find_if(aList.begin(), aList.end(), [](const auto& aPair)
    {
        return aPair.first == typeid(TType);
