...
public:
    ...
    template<typename TRhsRep>
        requires WmMath::ComparableEquality<TRep, TRhsRep>
    inline constexpr [[nodiscard]] bool operator==(const TRhsRep aRhs) const noexcept
    {
        return Rotation == aRhs;
    }
    template<typename TRhsRep>
        requires WmMath::ComparableStrong<TRep, TRhsRep>
    inline constexpr [[nodiscard]] auto operator<=>(const TRhsRep& aRhs) const noexcept
    {
        return Rotation <=> aRhs;
    }
    ...
    template<typename TRhsRep, typename TRhsRatio>
        requires WmMath::ComparableEquality<TRep, TRhsRep>
    inline constexpr [[nodiscard]] bool operator==(const SRotation<TRhsRep, TRhsRatio> aRhs) const noexcept
    {
        return *this == RotationCast<SRotation<TRhsRep, TRatio>>(aRhs).Rotation;
    }
    template<typename TRhsRep, typename TRhsRatio>
        requires WmMath::ComparableStrong<TRep, TRhsRep>
    inline constexpr [[nodiscard]] auto operator<=>(const SRotation<TRhsRep, TRhsRatio>& aRhs) const noexcept
    {
        return *this <=> RotationCast<SRotation<TRhsRep, TRatio>>(aRhs).Rotation;
    }
...
template<typename TRep, typename TRatio>
inline constexpr [[nodiscard]] SRotation<TRep, TRatio> operator+(SRotation<TRep, TRatio> aLhs, const TRep aRhs) noexcept
{
    return (aLhs += aRhs);
}
template<typename TRep, typename TRatio>
...
inline constexpr [[nodiscard]] SRotation<TRep, TRatio> operator+(SRotation<TRep, TRatio> aLhs, const SRotation<TFromRep, TFromRatio> aRhs) noexcept
{
    return (aLhs += aRhs);
}
template<typename TRep, typename TRatio, typename TFromRep, typename TFromRatio>
...
template <typename TToRotation, typename TFromRep, typename TFromRatio>
    requires std::_Is_specialization_v<TToRotation, SRotation>
inline constexpr [[nodiscard]] TToRotation RotationCast(const SRotation<TFromRep, TFromRatio> aRotation) noexcept
{
    ...
    static constexpr auto num = CommonFraction::num;
    static constexpr auto den = CommonFraction::den;

    static constexpr bool numIsOne = num == 1;
    static constexpr bool denIsOne = den == 1;
...
namespace WmMath
{
    template<typename TRep, typename TRatio, typename TReturnType = TRep>
        requires WmMath::has_floating_representation_v<TReturnType>
	inline [[nodiscard]] TRep Atan(const SRotation<TRep, TRatio> aRotation) noexcept
	{
		return WmMath::Atan(RotationCast<SRadian<TReturnType>>(aRotation).Rotation);
	}
	template<typename TRotation, typename TFromRep>
        requires WmMath::has_floating_representation_v<TFromRep>
	...
	template<typename TRep, typename TRatio, typename TReturnType = TRep>
        requires WmMath::has_floating_representation_v<TReturnType>
	inline [[nodiscard]] TReturnType Cos(const SRotation<TRep, TRatio> aRotation) noexcept
	{
		return WmMath::Cos(RotationCast<SRadian<TReturnType>>(aRotation).Rotation);
	}
    template<typename TRep, typename TRatio, typename TReturnType = TRep>
        requires WmMath::has_floating_representation_v<TReturnType>
	inline [[nodiscard]] TReturnType Sin(const SRotation<TRep, TRatio> aRotation) noexcept
	{
