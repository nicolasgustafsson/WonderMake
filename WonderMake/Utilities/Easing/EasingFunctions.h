...
namespace WmEasings
{
	[[nodiscard]] inline constexpr f32 Flip(const f32 aProgress) noexcept
	{
		return 1.f - aProgress;
	}

	template <WmMath::Interpolable T>
	[[nodiscard]] inline constexpr T Lerp(const SRange<T>& aRange, const f32 aProgress) noexcept
	{
		return WmMath::Lerp(aRange.Start, aRange.End, aProgress);
	}

	template <WmMath::Interpolable T>
	[[nodiscard]] inline constexpr T InvLerp(const SRange<T>& aRange, const f32 aProgress) noexcept
	{
		return (aProgress - aRange.Start) / (aRange.End - aRange.Start);
	}

	template <u32 Power = 2, WmMath::Interpolable T>
	[[nodiscard]] inline constexpr T SmoothStart(const SRange<T>& aRange, const f32 aProgress) noexcept
	{
		return Lerp(aRange, WmMath::Pow(aProgress, Power));
	}

	template <u32 Power = 2, WmMath::Interpolable T>
	[[nodiscard]] inline constexpr T SmoothStop(const SRange<T>& aRange, const f32 aProgress) noexcept
	{
		return Lerp(aRange, Flip(WmMath::Pow(Flip(aProgress), Power)));
	}

	template <WmMath::Interpolable T>
	[[nodiscard]] inline constexpr T Mix(const SRange<T>& aRange, T(*aFirstFunction)(const SRange<T>&, const f32), T(*aSecondFunction)(const SRange<T>&, const f32), const f32 aBlend, const f32 aProgress) noexcept
	{
		return Lerp(SRange<T>{ aFirstFunction(aRange, aProgress), aSecondFunction(aRange, aProgress) }, aBlend);
	}

	template <WmMath::Interpolable T>
	[[nodiscard]] inline constexpr T Crossfade(const SRange<T>& aRange, T(*aFirstFunction)(const SRange<T>&, const f32), T(*aSecondFunction)(const SRange<T>&, const f32), const f32 aProgress) noexcept
	{
	...
	[[nodiscard]] inline constexpr T SmoothStep(const SRange<T>& aRange, const f32 aProgress) noexcept
	{
	...
	template <WmMath::Interpolable T>
	[[nodiscard]] inline constexpr T Scale(const SRange<T>& aRange, T(*aFunction)(const SRange<T>&, const f32), const f32 aProgress) noexcept
	{
		return aFunction(aRange, aProgress) * aProgress;
	}

	template <WmMath::Interpolable T>
	[[nodiscard]] inline constexpr T ReverseScale(const SRange<T>& aRange, T(*aFunction)(const SRange<T>&, const f32), const f32 aProgress) noexcept
	{
		return aFunction(aRange, aProgress) * Flip(aProgress);
	}

	template <u32 Power = 2, WmMath::Interpolable T> requires (Power % 2 == 0)
	[[nodiscard]] inline constexpr T BellCurve(const SRange<T>& aRange, const f32 aProgress) noexcept
	{
	...
	template <WmMath::Interpolable T>
	[[nodiscard]] inline constexpr T QuadraticBezier(const SRange<T>& aRange, const T& aControl, const f32 aProgress) noexcept
	{
	...
	template <WmMath::Interpolable T>
	[[nodiscard]] inline constexpr T CubicBezier(const SRange<T>& aRange, const T& aControlFirst, const T& aControlSecond, const f32 aProgress) noexcept
	{
