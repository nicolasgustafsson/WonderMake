...
TEST(ConfigurationSerializationTests, deserialize_completes_with_parsing_error_when_given_non_json_string)
{
	...
	auto configSystem = MakeSystem<ConfigurationSystem>(std::make_tuple());

	DeserializeConfigurationJob::InjectDependencies(std::make_tuple(configSystem));
...
TEST(ConfigurationSerializationTests, serialize_completes_when_run)
{
	...
	SerializeConfigurationJob::InjectDependencies(std::make_tuple(configSystem));
